...
| secrets.GITHUB_TOKEN | 1. Personal center → Settings → Developer settings → Personal access tokens → Generate new token ( Name: GITHUB_TOKEN, Select: public_repo, Copy GITHUB_TOKEN's content ). 2. Op code center  → Settings → Secrets → New secret ( Name: RELEASES_TOKEN, Value: GITHUB_TOKEN's content ). |

...
| secrets.GITHUB_TOKEN | - 1. Personal center → settings → Developer settings → Personal access tokens → Create token ( Name:GITHUB_TOKEN, Select: public_repo, Copy GITHUB_TOKEN's content). - 2. op code center  → settings → Secrets → Add a new secret → ( Name:RELEASES_TOKEN, Value: GITHUB_TOKEN's content). |
